...
const MyOrders = () => {
  ...
  orders &&
    orders.forEach((item) => {
      ...
      console.log("Processing order:", item._id);
      console.log("Order items:", item.orderItems);
      
      // Calculate total quantity by summing up quantities of all items
      const totalQuantity = item.orderItems?.reduce((sum, orderItem) => {
        console.log("Processing order item:", orderItem);
        return sum + (orderItem.quantity || 0);
      }, 0) || 0;
      
      console.log("Total quantity for order", item._id, ":", totalQuantity);
